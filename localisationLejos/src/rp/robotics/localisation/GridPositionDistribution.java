...
public class GridPositionDistribution {
	...
	public boolean hasWinner(){
		for (int x = 0; x < m_gridMap.getGridWidth(); x++) {
			for (int y = 0; y < m_gridMap.getGridHeight(); y++) {
				if(this.getProbability(x, y) >= 0.6){
					System.out.println("winner!");				
					theWinner(x, y);	
					xPos = x;
					yPos = y;
					System.out.println(this.getProbability(x, y));
					return true;
				}
			}
		}
	...
	public void theWinner(int x, int y){
	...
	public void printHighest(){
		...
		for (int x = 0; x < m_gridMap.getGridWidth(); x++) {
			for (int y = 0; y < m_gridMap.getGridHeight(); y++) {
				if(this.getProbability(x, y) > temp){
					temp = this.getProbability(x, y);
					tempx = x;
					tempy = y;
				}
			}
		}
		System.out.println("Highest prob: "+temp);
		System.out.println("at: ("+tempx+", "+tempy+")");
	}
}
